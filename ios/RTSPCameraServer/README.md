...
**Scope (MVP):** single client, H.264 only, RTP delivered over the RTSP TCP
connection itself (interleaved mode, RFC 2326 §10.12) — no separate RTP/UDP ports
to configure, which matters given you're feeding this through a USB-C→RJ45
adapter. No audio, no multi-channel, no HTTP tunneling (see Android README for
what full parity would add).
...
- `ContentView.swift` — UI: camera preview, start/stop, shows the `rtsp://<ip>:8554/` URL to connect to.
- `CameraCaptureManager.swift` — `AVCaptureSession` wiring (back camera, 720p).
- `H264Encoder.swift` — `VTCompressionSession` wrapper; hardware H.264 encode, extracts SPS/PPS.
- `RTPPacketizer.swift` — RFC 6184 packetization (single-NAL + FU-A fragmentation).
- `RTSPServer.swift` — the RTSP server itself (`Network.framework`, TCP port 8554).
...
2. Delete the auto-generated `ContentView.swift` and `RTSPCameraServerApp.swift`, then drag in the six files from this folder's `Sources/` directory (check "Copy items if needed").
